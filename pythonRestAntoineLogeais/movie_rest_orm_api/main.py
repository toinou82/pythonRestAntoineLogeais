...
@app.delete("/movies/{movies_id}", response_model=schemas.Movie)
def delete_movie(movies_id: int, db: Session = Depends(get_db)):
    db_movie = crud.delete_movie(db=db, movies_id=movies_id)
    if db_movie is None:
...
def update_movie_actors(mid: int, sids: List[int], db: Session = Depends(get_db)):
    ...
    """
    db_movie = crud.update_movie_actor(db=db, movie_id=mid, actor_ids=sids)
    if db_movie is None:
        raise HTTPException(status_code=404, detail="Movie or Star not found")
    return db_movie
...
def read_stars_by_birthyear(y: int, db: Session = Depends(get_db)):
    # read items from database
    stars = crud.get_stars_by_birthyear(db=db, year=y)
    return stars
...
def read_count_stars_by_birthyear(db: Session = Depends(get_db)) -> List[Tuple[int, int]]:
    return crud.get_count_stars_by_birthyear(db=db)
